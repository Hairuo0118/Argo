...
public class VuMarkHandler : MonoBehaviour
{
    ...
    public void OnVuMarkDetected(VuMarkTarget target)
    {
		...
		var myNumericalId = target.InstanceId.NumericValue;
		previousIndex = (int)myNumericalId;
		//Debug.Log (myNumericalId);
	...
	// display the IDs of all existing targets in the panel
	void displayIDs(){
		Camera cam = DigitalEyewearARController.Instance.PrimaryCamera ?? Camera.main;
		...
		foreach (var bhvr in m_VuMarkManager.GetActiveBehaviours()) {
			Vector3 worldPosition = bhvr.transform.position;
			Debug.Log ("WORLD_POS: " + GetVuMarkId (bhvr.VuMarkTarget));
			Debug.Log (worldPosition);
			Vector3 camPosition = cam.transform.InverseTransformPoint(worldPosition);
			Debug.Log ("CAM_POS: " + GetVuMarkId (bhvr.VuMarkTarget));
			Debug.Log (camPosition);
			concat = concat + GetVuMarkId (bhvr.VuMarkTarget) + " ";
		}
		StartCoroutine(ShowPanelAfter(0.5f, concat, concat, concat, null));
	}
